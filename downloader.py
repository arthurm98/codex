...
async def mangadex_title_to_chapter_urls(url: str, fetcher: Fetcher) -> list[str]:
    parsed = urlparse(url)
    parts = [p for p in parsed.path.split("/") if p]
    ...
    while True:
        api = (
            ...
            f"?manga={manga_id}&translatedLanguage[]=en&order[chapter]=asc"
...
async def resolve_urls(args: argparse.Namespace, fetcher: Fetcher) -> list[str]:
    ...
    for u in urls:
        ...
        if site == "mangadex" and "/title/" in u:
            logging.info("Expandindo título do Mangadex: %s", u)
            chapter_urls = await mangadex_title_to_chapter_urls(u, fetcher)
